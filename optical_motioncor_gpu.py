...
try:
    import cupy as cp
    CUPY_AVAILABLE = True
    print("CUDA acceleration enabled with CuPy")
except ImportError:
...
def calculate_drift_correlation_gpu(image1, image2, upsample_factor=10):
    ...
    try:
        # 将数据移到GPU
        img1_gpu = cp.asarray(image1)
        img2_gpu = cp.asarray(image2)
        
        # 使用scikit-image的相位相关（但数据在GPU上）
        shift, error, diffphase = phase_cross_correlation(
            cp.asnumpy(img1_gpu), cp.asnumpy(img2_gpu), upsample_factor=upsample_factor
        )
...
def batch_phase_cross_correlation_gpu(images_batch1, images_batch2, upsample_factor=10):
    ...
    try:
        # 将批次数据移到GPU
        batch1_gpu = cp.asarray(images_batch1)
        batch2_gpu = cp.asarray(images_batch2)
        
        # 分别处理每个图像对
        shifts = []
        for i in range(len(batch1_gpu)):
            shift, error, diffphase = phase_cross_correlation(
                cp.asnumpy(batch1_gpu[i]), cp.asnumpy(batch2_gpu[i]), 
...
def iterative_drift_correction(frames, max_iterations=10, threshold=0.5, 
                             upsample_factor=50, batch_size=30, use_gpu=True):
    """
    ...
    print(f"Starting iterative drift correction (max {max_iterations} iterations, threshold {threshold} pixels, batch_size {batch_size}, GPU: {use_gpu and CUPY_AVAILABLE})")
...
def main():
    ...
    # 检查GPU可用性
    use_gpu = CUPY_AVAILABLE and not args.no_gpu
    print(f"GPU acceleration: {'Enabled' if use_gpu else 'Disabled'} (CuPy available: {CUPY_AVAILABLE})")
